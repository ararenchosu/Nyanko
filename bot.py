import discord
from discord import ui, Embed, Color, ButtonStyle
...
ADMIN_USER_IDS = [123456789012345678]  # 👈 管理者のユーザーID
JISSEKI_CHANNEL_ID = 1546928125231767633
...
class ServiceModal(ui.Modal, title="📋 ご依頼情報入力"):
    def __init__(self, service_name: str, price: int, is_clone=False):
        ...
        self.add_item(ui.TextInput(label="📱 引き継ぎコード", placeholder="XXXX-XXXX-XXXX-XXXX", required=True))
        self.add_item(ui.TextInput(label="🔑 認証番号", placeholder="12345678", required=True))
        if is_clone:
            self.add_item(ui.TextInput(label="📝 備考（任意）", required=False, style=discord.TextStyle.long))
...
class AdminMenuView(ui.View):
    ...
    async def price_btn(self, interaction, button):
    ...
    async def ch_btn(self, interaction, button):
        await interaction.response.send_message(
...
class SingleItemView(ui.View):
    ...
    async def select_items(self, interaction, select):
        items_selected = select.values
    ...
    def __init__(self, guild_id: int):
        ...
        self.select_items.options = opts
...
class SetItemView(ui.View):
    ...
    async def select_set(self, interaction, select):
...
# ✅ コマンド
# ======================================================
@bot.tree.command(name="panel", description="代行サービスパネルを表示")
async def panel_cmd(interaction):
    embed = Embed(
...
@bot.tree.command(name="setprice", description="項目別価格を設定（管理者のみ）")
async def setprice_cmd(interaction, key: str, price: int):
    if not is_admin(interaction.user.id):
        await interaction.response.send_message("❌ 管理者専用コマンドです。", ephemeral=True)
        return
    if key not in ITEM_CONFIG:
        await interaction.response.send_message(f"❌ 項目キー `{key}` は存在しません。", ephemeral=True)
        return
    ov = load_price_overrides(interaction.guild_id)
    ov[key] = price
    save_price_overrides(interaction.guild_id, ov)
    await interaction.response.send_message(f"✅ `{ITEM_CONFIG[key]['label']}` を **{price}円** に変更しました。", ephemeral=True)


@bot.tree.command(name="setjisseki", description="実績チャンネル設定（管理者のみ）")
async def setjisseki_cmd(interaction, channel_id: str):
    if not is_admin(interaction.user.id):
        await interaction.response.send_message("❌ 管理者専用コマンドです。", ephemeral=True)
        return
...
@bot.tree.command(name="admin_list", description="全項目キー一覧（管理者のみ）")
async def admin_list_cmd(interaction):
    if not is_admin(interaction.user.id):
        await interaction.response.send_message("❌ 管理者専用コマンドです。", ephemeral=True)
        return
    text = "\n".join(f"`{k}` — {v['label']} ({v['price']}円)" for k,v in ITEM_CONFIG.items())
    await interaction.response.send_message(embed=Embed(title="📋 全項目キー一覧", description=text[:4000], color=0xcccccc), ephemeral=True)
...
if __name__ == "__main__":
    ...
    if not TOKEN or TOKEN in ("", "ここにトークン"):
        print("❌ DISCORD_TOKEN を環境変数に設定してください")
        exit(1)
    bot.run(TOKEN)
